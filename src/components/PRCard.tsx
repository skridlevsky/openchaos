...
export function PRCard({ pr, rank }: PRCardProps) {
  ...
  const isSexNumber = pr.votes === 67 || pr.votes === -67;
  const hasConflict = !pr.isMergeable;
  const cardClass = hasConflict
    ? `pr-card pr-card-normal pr-card-conflict ${isSexNumber ? "sixtynine-shake" : ""}`
    : `pr-card ${rank === 1 ? 'pr-card-leading' : 'pr-card-normal'} ${isSexNumber ? "sixtynine-shake" : ""}`;
  return (
    <table
      width="100%"
      border={2}
