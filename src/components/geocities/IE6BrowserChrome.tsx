...
export function IE6BrowserChrome({ children }: IE6BrowserChromeProps) {
  ...
  useEffect(() => {
    if (user) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- sync title with auth state
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reset startup sound flag
            ...
            onClick={() => {
              if (user) {
                ...
                if (audio) {
                  ...
                  audio.addEventListener('error', redirect);
